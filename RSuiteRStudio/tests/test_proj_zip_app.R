...
test_that_shiny_app(app_dir, "Correct pkgzip creation", {
  # Prepare project
  ...
  # Install dependencies (required before zipping)
  ...
  app$setInputs(zip_version = "1.0")
  app$setInputs(project_folder = params$prj_path)
  app$setInputs(start_btn = "click", wait_ = FALSE, values_ = FALSE)
  app$refresh()
  app$stop()

  expect_true(file.exists(file.path(app_dir, "TestProject_1.0x.zip")))
  unlink(file.path(app_dir, "TestProject_1.0x.zip"), recursive = TRUE, force = TRUE)
})
